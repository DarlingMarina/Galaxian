...
*****************************************************************************/
void test_starfield(Surface & surface)
{
    ...
    while(1)
    {
        ...
        surface.lock();
        surface.fill(BLACK);
	surface.put_circle(x, y, radius, r, g, b);
	surface.unlock();
	surface.flip();
...
void test_circle() //star field template?
{
	Surface surface(W, H);
	Event event;
...
void test_keyboard()
{
	...
	while (1)
	{
                ...
                if (keypressed[RIGHTARROW])   
		{
			...
			surface.lock();
			surface.put_line(0, 0, x, y, rand() % 256, rand() % 256 , rand() % 255);
			surface.unlock();
			surface.flip();
			...
			delay(10);
                ...
                // left, right, up arrow, space is whats needed.
                // image filefor  own spaceship so it goes back fourth at the bottom
                // image file for rocket or something. firing is space. make rocket fire from exactly the front/top
                // of spaceship even when you immediatly move the spaceship elsewhere
	}
	return;
}
...
void test_image()
{
    ...
    int speed = 5;
    int dx = speed;
    int dy = 0; // if it was 1 then it would move down
	...
	while (1)
	{
        ...
        rect.x += dx;
        rect.y += dy; //as it moves left to right it also moves down
        // but we dont have the logic to make him reappear 
        /*
          rect.x += dx;
        ...
        */
        ...
        if (rect.y > 400) // now it will go up and down
        {
            dy = -dy;
        }
        else if(rect.y < 0)
...
int main(int argc, char* argv[])
{
    ...
    //test_gamepad(); // NEW 2013
        ...
        /********************************************************************************
        /* main game loop below here
        / *******************************************************************************/

        Event event;
        Surface surface(W, H);

        // At the top of the file, #include<vector>
        // std::vector< Star > starField;
        // x = 0, y = 0
        // for (int i = 0; i < 100; ++i)
        // {
        //     Star s = new Star(surface, x, y);
        //     x += 5, y += 5;
        ...
        while (1)
	{
            ...
            if (type == QUIT)
            {
                ...
                if (gamestate == 1) // play the game
                {
                     ...
                     // get keyboard input (LeftArrow, RightArrow, Space for shooting maybe?)

                    
                     KeyPressed keypressed = get_keypressed();
                     if (keypressed[SPACE])
                     {
                         gamestate = 2; // for now this is just so you can loop through the
                                        // screens
                     }
                     
                     // make moves accordingly
                     test_starfield(surface);

                         // loop over starfield
                         // call starObject.move()
                     
                     // adjust score/health
                     // update surface
                         // loop over starField
                         // call starObject.draw()
                }
...
// TEST BACKGROUN stAR DEPTH
// CREATE ILLUSION OF DEPTH WITH LARGER AND BRiGHTeR STARS CLOsEST TO YOU
// AND GOING DOWN FAStER. THEN SMALL LeSS BRIGHT AND FALLING SLOWER
// BEhIND THE LARGER ONES. WITH LES R, G, B
